...
HAL_StatusTypeDef SHT31_ReadTemperature(SHT31_type *sht31, double *temperatureValue){
	uint16_t data = 0u;
	if(HAL_I2C_Master_Receive(sht31->I2cHandle, SHT31_ADDRESS_VSS << 1, sht31->tempratureRAW, 3, 5) == HAL_OK){
...
HAL_StatusTypeDef SHT31_ReadHumihity(SHT31_type *sht31, float *humihityValue){
	uint16_t data = 0u;
	if(HAL_I2C_Master_Receive(sht31->I2cHandle, SHT31_ADDRESS_VSS << 1, sht31->humihityRAW, 2, 5) == HAL_OK){
